 ...
 select * from (
 select symbol, SUM(days) days, (SUM(pnl)-SUM(txn_costs))*100/AVG(avgprice) as nett_pct, SUM(nett) as nett_val, SUM(pct_pnl) - (SUM(days) * @txncost_pct) as pct_pnl, SUM(days) * @txncost_pct as pct_txncost,  AVG(avgprice) as avgprice, AVG(val) val from (
