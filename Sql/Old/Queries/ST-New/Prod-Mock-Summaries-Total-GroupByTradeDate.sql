...
set @margin_fraction = 0.25
		 ...
		 --,contract_name
		 ,r1
		 --,CAST(status_update_time as DATE) as date1
		 ,algo_id
		 ...
		 ,((SUM(actual_profit) * 100) /((SUM(average_trade_price * quantity * @margin_fraction))/count(*))) as ROI
		  ...
		  ,SUM(brokerage * .67 + actual_profit) as PROFIT_OPTIM
...
--DATEDIFF(d, CAST(status_update_time as DATE), GETDATE()) = 1
		 ...
		 group by 
  --CAST(status_update_time as DATE)
  r1
  ,algo_id
  ...
  order by 
  r1, ROI desc
  --CAST(status_update_time as DATE) desc