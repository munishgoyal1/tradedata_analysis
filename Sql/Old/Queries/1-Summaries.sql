...
--insert into @contracts select * FROM FN_HIGH_TICKS_FUT_5_RANDOM_STOCKS() 
    --insert into @contracts select * FROM ('YESBANK', 'DLF') 
        
--insert into @contracts select top(5) * FROM  dbo.st_selected_contracts_book order by newid() --limit 5 
		 ...
		 ,market_direction_percentage --algo_id,
		  ...
		  --,SUM(brokerage) as ACTUAL_BROK
		   ...
		   and (number_of_ticks/num_days > 320)
		   --and r2 not like 'Indices%'
		   and r2 = 'IEOD-1min'
		   --and r2 like 'IEOD%'
		   and market_direction_percentage  = 1.5--between 1.1 and 1.5
		   ...
		   --and contract_name in ('YESBANK', 'DLF')
		  group by  
		  r1, 
		  algo_id 
