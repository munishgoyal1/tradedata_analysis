...
select * from(
		...
		CAST(eod.trade_date AS DATE) as TR_DATE
			  ...
			  ,SUM(((eod.brokerage * @optimum_brokerage_discount) + eod.actual_profit) * 100 / (eod.max_price * eod.quantity)) as DAY_OP_PERC --optimum profit
	    ...
	    where 
			  eod.algo_id in (117) and eod.market_direction_percentage = 0.7  -- staban 152 0.6
