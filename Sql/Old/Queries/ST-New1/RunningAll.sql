...
CAST(eod.trade_date AS DATE) as TR_DATE
      ...
      ,SUM(eod.actual_profit) as DAY_AP
        ,(SELECT SUM(eod1.actual_profit)
		  FROM [StockTrader-QA].[dbo].[st_trading_eod_statsbook] as eod1
			  ...
			  and eod1.contract_name in ('CNXBAN', 'NIFTY', 'STABAN', 'INFTEC', 'SAIL', 'MCDOWE', 'JAIASS')
    ...
    --    ) as INVST
	  ...
	  where 
	  --eod.algo_id = @algo_id
	  eod.algo_id in (17, 18, 19, 40, 41, 42)
	  --and eod.market_direction_percentage = @market_direction_percentage
	  and eod.market_direction_percentage in (0.3, 0.4, 0.5, 0.6)
	  and eod.r2 = @r2
	  --and eod.r1 = @r1
	  --and eod.number_of_ticks > @ticks_per_day
	  --and eod.contract_name = @contract
	  and eod.contract_name in ('CNXBAN', 'NIFTY', 'STABAN', 'INFTEC', 'SAIL', 'MCDOWE', 'JAIASS')
