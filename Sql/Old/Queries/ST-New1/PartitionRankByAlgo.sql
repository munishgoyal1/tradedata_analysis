...
Select TOP 2147483647 * from
(
    select VIEW1.*, Rank() over (Partition BY VIEW1.ALGO ORDER BY VIEW1.AP DESC ) AS Rank
    --CHECKSUM(VIEW1.ALGO, VIEW1.CONTRACT) ORDER BY VIEW1.AP DESC ) AS Rank
from
    ( Select 
		 count(*) as RECORDS,
...
order by VIEW3.ALGO asc, VIEW3.AP desc
